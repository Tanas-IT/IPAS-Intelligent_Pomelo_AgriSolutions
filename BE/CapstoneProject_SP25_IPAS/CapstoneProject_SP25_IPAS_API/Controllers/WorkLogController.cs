...
namespace CapstoneProject_SP25_IPAS_API.Controllers
{
    ...
    public class WorkLogController : ControllerBase
    {
        ...
        [HttpGet(APIRoutes.WorkLog.StatisticWorkLog, Name = "StatisticWorkLog")]
        public async Task<IActionResult> StatisticWorkLog([FromQuery] int? farmId)
        {
            try
            {
                if (!farmId.HasValue)
                    farmId = _jwtTokenService.GetFarmIdFromToken() ?? 0;
                var result = await _workLogService.TaskStatics(farmId.Value);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet(APIRoutes.WorkLog.FilterEmployeeByWorkLogId, Name = "FilterEmployeeByWorkLogId")]
        [HybridAuthorize($"{nameof(RoleEnum.ADMIN)},{nameof(RoleEnum.OWNER)},{nameof(RoleEnum.MANAGER)},{nameof(RoleEnum.EMPLOYEE)}")]
        [CheckUserFarmAccess]
        //[FarmExpired]
        public async Task<IActionResult> FilterEmployeeByWorkLogId([FromQuery] int? workTypeId, int? farmId)
        {
            try
            {
                if (!farmId.HasValue)
                    farmId = _jwtTokenService.GetFarmIdFromToken() ?? 0;
                var result = await _workLogService.FilterEmployeeByWorkSkill(workTypeId, farmId.Value);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet(APIRoutes.WorkLog.GetDependencyWorkLog, Name = "GetDependencyWorkLog")]
        [HybridAuthorize($"{nameof(RoleEnum.ADMIN)},{nameof(RoleEnum.OWNER)},{nameof(RoleEnum.MANAGER)},{nameof(RoleEnum.EMPLOYEE)}")]
        [CheckUserFarmAccess]
        //[FarmExpired]
        public async Task<IActionResult> GetDependencyWorkLog([FromRoute] int id)
        {
            try
            {
                var result = await _workLogService.GetDependentWorkLog(id);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet(APIRoutes.WorkLog.FilterEmployeeByTarget, Name = "FilterEmployeeByTarget")]
        [HybridAuthorize($"{nameof(RoleEnum.ADMIN)},{nameof(RoleEnum.OWNER)},{nameof(RoleEnum.MANAGER)},{nameof(RoleEnum.EMPLOYEE)}")]
        [CheckUserFarmAccess]
        //[FarmExpired]
        public async Task<IActionResult> FilterEmployeeByTarget([FromQuery] string? target, int? farmId)
        {
            try
            {
                if (!farmId.HasValue)
                    farmId = _jwtTokenService.GetFarmIdFromToken() ?? 0;
                var result = await _workLogService.FilterEmployeeByTarget(target, farmId.Value);
                return Ok(result);
            }
            catch (Exception ex)
            {
        ...
        [HttpGet(APIRoutes.WorkLog.DeleteNoteOfWorkLog, Name = "DeleteNoteOfWorkLog")]
        [HybridAuthorize($"{nameof(RoleEnum.ADMIN)},{nameof(RoleEnum.OWNER)},{nameof(RoleEnum.MANAGER)},{nameof(RoleEnum.EMPLOYEE)}")]
        [CheckUserFarmAccess]
        //[FarmExpired]
        public async Task<IActionResult> DeleteNoteOfWorkLog([FromQuery] int workLogId, [FromQuery] int userId)
        {
            try
            {
                var result = await _workLogService.DeleteNoteOfWorkLog(userId, workLogId);
                return Ok(result);
            }
            catch (Exception ex)
            {
