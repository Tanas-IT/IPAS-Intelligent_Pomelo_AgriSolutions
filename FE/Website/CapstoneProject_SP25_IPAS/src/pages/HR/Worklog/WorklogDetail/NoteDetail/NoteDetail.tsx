...
const NoteDetail: React.FC<NoteDetailProps> = ({ note, onClose }) => {
  ...
  const images = note.listResources
    .filter((resource) => resource.resourceURL.match(/\.(jpeg|jpg|gif|png)$/i))
    .map((resource) => resource.resourceURL);

  const videos = note.listResources
    .filter((resource) => resource.resourceURL.match(/\.(mp4|mov|avi)$/i))
    .map((resource) => resource.resourceURL);
