...
const AddPlanByProcess = () => {
    ...
    const fetchPlans = async () => {
      ...
      try {
        ...
        if (response.statusCode === 200 && response.data) {
          ...
          const subProcessesListPlan =
            selectedProcess.subProcesses?.flatMap(
    ...
    if (frequency === "None") {
      if (dates.length === 1) {
        const isDateRangeAdjusted =
          startDate.isSame(dates[0], "day") && endDate.isSame(dates[0], "day");
        ...
        const isDateRangeAdjusted =
          startDate.isSame(minDate, "day") && endDate.isSame(maxDate, "day");
        if (!isDateRangeAdjusted) {
          Modal.confirm({
            title: "Adjust Date Range",
            content: `The selected dates range from ${minDate.format(
              "YYYY-MM-DD",
            )} to ${maxDate.format(
              "YYYY-MM-DD",
            )}. Do you want to adjust the date range to match these dates?`,
            onOk: async () => {
      ...
      if (selectedPlan.masterTypeId !== null && selectedPlan.masterTypeId !== undefined) {
        try {
          const response = await worklogService.getEmployeesByWorkSkill(
            Number(getFarmId()),
            selectedPlan.masterTypeId,
          );
          if (response.statusCode === 200) {
            setFilteredEmployees(response.data);
          } else {
            toast.warning("Failed to fetch employees for this work type.");
            return;
          }
        } catch (error) {
          toast.warning("Failed to fetch employees. Please try again later.");
          return;
        }
      } else {
        Modal.error({
        ...
        if (item.type === "plan" && item.planId === planId) {
          return {
            ...
            schedule: {
              ...
              customDates:
                frequency === "None"
                  ? customDates.map((date: Dayjs) => date.format("YYYY-MM-DD"))
                  : [],
  ...
  const handleSubmit = async () => {
    try {
      ...
      if (!processId) {
        toast.warning("No process selected. Please select a process.");
        return;
      }

      if (planTargetType === 1 && growthStage.length === 0) {
        toast.warning(
          "Please select at least one Growth Stage Name for Land Plot/Land Row/Plant target.",
        );
        return;
      }
      ...
      if (planTargetType === 1 && growthStage.length > 0) {
        ...
        if (!Array.isArray(planTargetModels) || planTargetModels.length === 0) {
          toast.warning("Please add at least one target in the Plan Target section.");
          return;
        }
      }
      ...
      if (allPlans.length === 0) {
        toast.warning("No valid plans found to submit.");
        return;
      }
        ...
        <Form form={form} layout="vertical">
          ...
          >
            <Form
              form={scheduleForm}
              onFinish={(values) => {
                const { dateRange, timeRange, frequency, dayOfWeek, dayOfMonth, customDates } =
                  values;
                ...
                if (
                  frequency === "Weekly" &&
                  (!isDaySelectorSaved || !dayOfWeek || dayOfWeek.length === 0)
                ) {
                  Modal.error({
                    title: "Invalid Input",
                    content:
                      "Please click Save in the DaySelector to confirm the selected days of the week.",
                    okText: "Close",
                  });
                  return;
                }
                ...
                if (
                  frequency === "Monthly" &&
                  (!isDaySelectorSaved || !dayOfMonth || dayOfMonth.length === 0)
                ) {
                  Modal.error({
                    title: "Invalid Input",
                    content:
                      "Please click Save in the DaySelector to confirm the selected days of the month.",
                    okText: "Close",
                  });
                  return;
                }
            ...
            >
              ...
              <Form.Item>
                <Button
                  type="primary"
                  disabled={
                    (frequency === "Weekly" || frequency === "Monthly") && !isDaySelectorSaved
                  }
                  // onClick={() => {
                  //     if (frequency !== "None") {
                  //         scheduleForm.setFields([{ name: "customDates", errors: [] }]);
