...
const UpdatePlanTarget = ({
  ...
  useEffect(() => {
    if (initialValues.length > 0) {
      ...
      setSelectedPlants(
        formattedValues.map((target) => target.plants.map((plant) => plant.plantId)),
      );
      setSelectedPlantLots(
        formattedValues.map((target) => target.plantLots.map((lot) => lot.plantLotId)),
      );
      setSelectedGraftedPlants(
        formattedValues.map((target) =>
          target.graftedPlants.map((grafted) => grafted.graftedPlantId),
        ),
      );
    }
  }, [initialValues, form]);
  ...
  const renderFields = (unit: string, index: number) => {
    ...
    const isUnitDisabled =
      (unit === "graftedplant" || unit === "plantlot") &&
      ...
      <Col span={6}>
        ...
        >
          ...
          >
            {selectedTarget
              ?.find((target) => target.landPlotId === selectedLandPlot)
              ?.rows.find((row) => row.landRowId === selectedLandRow[0])
              ?.plants.map((plant) => (
