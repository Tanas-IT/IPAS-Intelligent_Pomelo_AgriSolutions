...
import {
  ...
  PestReportRequest,
  PestReportResponse,
} from "@/types/pestDetection";

export const predictDisease = async (
  formData: FormData
): Promise<ApiResponse<PestDetectionResult[]>> => {
  const res = await axiosAuth.axiosMultipartForm.post(
    "ai/predict-disease-by-file",
    formData
  );
...
export const createReport = async (
  data: PestReportRequest
): Promise<ApiResponse<PestReportResponse>> => {
  const res = await axiosAuth.axiosMultipartForm.post(
    "report-of-user/create",
    data
  );
...
export const predictDiseaseByUrl = async (
  imageURL: string
): Promise<ApiResponse<PestDetectionResult[]>> => {
  const res = await axiosAuth.axiosJsonRequest.post(
    "ai/predict-disease-by-url",
    { imageURL }
  );
  const apiResponse = res.data as ApiResponse<PestDetectionResult[]>;
  return apiResponse;
