...
const GraftedNoteScreen: React.FC = () => {
  const navigation = useNavigation<RootStackNavigationProp>();
  const route = useRoute();
  const { graftedPlantId, historyId, initialData } = route.params as {
  ...
  const onSubmit = async (data: NoteFormData) => {
    ...
    try {
      const payload = {
        graftedPlantNoteId: historyId ?? 0,
        graftedPlantId,
        userId,
      ...
      let response;
      if (isEditMode) {
        response = await GraftedPlantService.updateGraftedPlantGrowthHistory(
          payload
        );
      } else {
        response = await GraftedPlantService.createGraftedPlantGrowthHistory(
          payload
        );
      }
...
export default GraftedNoteScreen;
