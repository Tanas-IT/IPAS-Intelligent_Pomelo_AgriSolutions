...
const AvatarUploader: React.FC = () => {
  ...
  const { avatarUrl, userId, roleId, accessToken, refreshToken, setAuth } = useAuthStore();
  ...
  const pickImage = async () => {
    ...
    if (!result.canceled && result.assets[0].uri) {
      ...
      try {
        ...
        const res = await userService.updateAvatarUser(mediaFile, Number(userId));
        ...
        Toast.show({ type: 'error', text1: 'Error', text2: 'Failed to upload avatar' });
      } finally {
