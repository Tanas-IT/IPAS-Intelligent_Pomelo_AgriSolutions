...
const NoteFormScreen: React.FC = () => {
  ...
  const { plantId, historyId, initialData } = route.params as {
    plantId: number;
    historyId?: string; // Có historyId là Update, không có là Add
    initialData?: NoteFormData; // Dữ liệu ban đầu khi Update
  };

  const isEditMode = !!historyId; // Chế độ chỉnh sửa nếu có historyId
  ...
  const onSubmit = async (data: NoteFormData) => {
    ...
    const payload = {
      ...data,
      plantId,
      noteTaker: 'Current User', // Thay bằng logic lấy user thực tế
    };
    ...
    // if (isEditMode) {
    //   await updateNote(historyId, payload); // PATCH hoặc PUT
    // } else {
    //   await addNote(payload); // POST
    // }
  };
