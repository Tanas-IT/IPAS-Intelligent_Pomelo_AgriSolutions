...
import { View, Text, StyleSheet, FlatList, Image, SafeAreaView, ScrollView, Animated, TouchableOpacity } from 'react-native';
import { Agenda, DateData, AgendaEntry, AgendaSchedule, TimelineEventProps, Calendar } from 'react-native-calendars';
...
export default function WorklogScreen() {
  ...
  const [selectedDate, setSelectedDate] = useState<string>(eventDates[0] || new Date().toISOString().split('T')[0]);
  const calendarHeight = useRef(new Animated.Value(MIN_CALENDAR_HEIGHT)).current;
  ...
  const getWeekRange = (selectedDate: string) => {
    ...
    startOfWeek.setDate(date.getDate() - dayOfWeek + (dayOfWeek === 0 ? -6 : 1)); // bắt đầu từ t2
    ...
    return (
      <View style={styles.eventRow}>
        ...
        <TouchableOpacity
          ...
          onPress={() => navigation.navigate(ROUTE_NAMES.WORKLOG.WORKLOG_DETAIL, { worklogId: item.id.toString() })}
        >
    ...
    return (
      <View style={{ marginBottom: 20 }}>
        ...
        {eventsForDate.length > 0 ? (
          ...
          <View style={styles.eventRow}>
            <View style={styles.timeColumn}>
              <TextCustom style={[styles.timeText, { color: theme.colors.background }]}>g</TextCustom>
            </View>
            <View style={[styles.eventDetails, { borderLeftColor: 'white' }]}>
              <TextCustom style={[styles.eventName, { color: '#888' }]}>No Activities</TextCustom>
            </View>
          </View>
        )}
      </View>
  ...
  const onHandlerStateChange = (event: { nativeEvent: { state: number; translationY: number } }) => {
    if (event.nativeEvent.state === State.END) {
      ...
      const newHeight = translationY > 0 ? MAX_CALENDAR_HEIGHT : MIN_CALENDAR_HEIGHT;
                    ...
                    {weekDates.map((date) => (
                      <View key={date} style={styles.weekDateWrapper}>
                        ...
                        >
                          <TextCustom style={[
                            styles.weekDateText,
                            date === selectedDate && styles.selectedWeekDateText,
                          ]}>{new Date(date).getDate()}</TextCustom>

                          {eventDates.includes(date) && <View style={styles.eventDot} />}
                        </TouchableOpacity>
                      </View>
                    ))}
                  </View>
