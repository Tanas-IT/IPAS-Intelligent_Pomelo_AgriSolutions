...
const VideoThumbnail: React.FC<{ videoUrl: string; onSelect: (url: string) => void }> = ({
  ...
  return (
    <TouchableOpacity onPress={() => onSelect(videoUrl)} style={styles.thumbnailContainer}>
      <VideoView player={player} style={styles.videoThumbnail} nativeControls={false} />
      <CustomIcon
        name="play-circle"
        size={30}
        color="#fff"
...
const NoteDetailModal2: React.FC<NoteDetailModalProps> = ({ visible, resources, onClose }) => {
...
export default NoteDetailModal2;